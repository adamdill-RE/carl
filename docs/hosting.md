...
| Value | RESM | New app |
| --- | --- | --- |
| Public URL | `https://www.reshiftmanager.com/resm/` | `https://www.reshiftmanager.com/carl/` |
| cPanel account / home | `/home/reshiftmanager` | `/home/reshiftmanager` (shared with RESM) |
| Web directory | `/home/reshiftmanager/public_html/resm` | `/home/reshiftmanager/public_html/carl` |
| App directory (outside `public_html`) | `/home/reshiftmanager/resm-app` | `/home/reshiftmanager/carl-app` |
| `app.base_path` | `/resm/` | `/carl/` |
| Session cookie name | `RESMSESS` | `CARLSESS` |
| Session cookie path | `/resm/` | `/carl/` |
| Database host | `152.160.193.196` (account-wide) | same, unless the new app is on a different account |
| Database name / user | `reshiftmanager_resm` (cPanel prefixes both with the account name) | `reshiftmanager_carl` |
| Env var prefix | `RESM_` | `CARL_` |
| Error log | `/home/reshiftmanager/logs/php.error.log` | same (account-wide) |
| Status / setup keys | `status_key`, `setup_key` in the gitignored local config | `status_key`, `setup_key`, `cron_key`, `diag_key` in `config/local.php` |
